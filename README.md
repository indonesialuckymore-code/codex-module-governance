...
当前版本是 `0.12.0`，已完成 C01–C12，包括自然语言前台、唯一中央路由器、工程总账、任务包、占用、验收、裁定、恢复、任务调度及外部 Skill 登记与适配。
...
它已经具备 Boss 自然语言入口和 Codex 任务两段式调度合同，但尚未完成 C13 干净环境验收；因此不能宣称“下载后立即可用”。它仍不具备外部系统原生写前锁，也不直接修改业务系统。
...
通过仅代表 C01–C12 产品能力完整，不代表 C13 已完成，也不代表任何具体业务链路已经验收。
