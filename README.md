...
当前 Git 私有稳定版为 `0.21.0`（标签 `v0.21.0`）。Boss 的中央入口已收敛为唯一的“中央工作台”，而 C09 中央控制与 C12 自然语言解析继续作为内部协议。新建或复用的任务窗口、一级子 Agent 必须由原生运行参数指定 `gpt-5.6-terra` 并留下凭据；它不迁移工程总账、不新建第二套任务状态，也不改变既有派发、回传和验收链。
...
- C10 复用中央启动图批准，按波次批量派发，强制绑定保存项目并核对 `任务ID｜业务名称｜G代际`；同一窗口最多承接两项串行任务，第二项完成后强制退役；任务窗口可在原范围内主动补派 0–3 个子 Agent，但必须结构化回传、父窗口读证和质量闸门后才可进入 C06；
...
先按 [安装说明](docs/INSTALLATION.md) 从固定稳定标签 `v0.21.0` 注册本人 GitHub Marketplace，再安装 `codex-module-governance@qianyi-codex-governance`。私有仓库用户必须先获得 GitHub 访问权限；安装或升级后需要新开 Codex 任务加载新版能力。
