...
当前 Git 私有稳定版仍为 `0.22.0`（标签 `v0.22.0`）；当前源码是待发布的 `0.22.1` 新项目启动修复候选版。它不改变原架构：Boss 仍只使用“中央工作台”，C03 仍是唯一详细状态源。修复点是：首次启动时当前窗口必须成为唯一 `CURRENT_CENTRAL`；旧中央不能继续写账；新 Git 任务先建立保存项目归属，再交接到标准 worktree；任务窗口必须证明无法访问中央私有治理目录。
...
- C06/C08 回传票据、自动入箱、单槽独立验收、十类证据独立读回、Boss 最终 `DONE` 门槛及不可变续派触发；
- C07 原窗口阻塞、顾问建议、Boss 决定与原窗口回传边界；
- C08 失联冻结、中央/裁定计划换代、逻辑角色路由、持久任务事件箱、恢复决定和受控占用释放；
- C09 唯一中央一次读完整大纲，集中列出 Boss 决策、全部任务、依赖图和并行波次；
- C10 复用中央启动图批准，按波次批量派发，强制绑定保存项目并核对 `任务ID｜业务名称｜G代际`；Terra 与 `WORKTREE_SCOPED` 受限权限缺一不可，完整访问拒绝开工；同一窗口最多承接两项串行任务，第二项完成后强制退役；任务窗口可在原范围内主动补派 0–3 个子 Agent，但必须结构化回传、父窗口读证和质量闸门后才可进入 C06；
...
| 任务窗口 / 一级子 Agent 运行模型 | 强制 `gpt-5.6-terra`；无原生参数与回执不得进入 `IN_PROGRESS` |
| 任务窗口 / 一级子 Agent 运行权限 | 强制 `WORKTREE_SCOPED`；`:danger-full-access`、缺权限证据或仅有文字承诺不得进入 `IN_PROGRESS` |
...
| 新任务项目归属 | 必须绑定 Codex 保存项目；Git 任务先在该项目 `local` 环境创建，读回项目 ID 后再交接到标准 worktree；禁止先建空归属 worktree 候选或反复重建 |
| 任务窗口私有数据边界 | 必须声明唯一可写项目根，并证明中央治理数据目录为 `DENIED`；全局 `workspace-write` 不再是合格任务权限 |
...
./scripts/verify-c14.sh
```

通过代表 C01–C14 产品包、干净安装和回退合同已验证，不代表任何具体业务链路已经验收。
