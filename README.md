...
当前 Git 私有稳定版为 `0.22.0`（标签 `v0.22.0`）。Boss 的中央入口仍是唯一的“中央工作台”，C09 中央控制与 C12 自然语言解析继续作为内部协议。新建或复用的任务窗口、一级子 Agent 必须同时通过原生 Terra 参数、工作区受限权限、保存项目和真实运行 ID 读回；新增原生任务清单、批量等待、换代 fork/handoff、标题/置顶/归档投影，但 C03 仍是唯一详细状态源。
