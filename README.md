...
Git 标签稳定安装基线仍是 `0.13.0`；当前本地施工版为 `0.15.0`，新增 C00 全项目大纲、中央启动图、范围化批量批准、项目绑定派发、稳定任务身份、中央/裁定换代和持久任务事件箱。
...
- C10 复用中央启动图批准，按波次批量派发，强制绑定保存项目并核对 `任务ID｜业务名称｜G代际`；worktree 项目 ID 丢失时对同一任务自动走项目本地往返交接，再验证真实运行归属；
