...
稳定安装基线仍是 `0.13.0`。当前 `main` 新增待下版发布的 C00 通用施工大纲规划、C09 中央启动图、范围化批量批准和 C10 项目绑定派发；既有 C01–C13 工程总账、占用、验收、裁定、恢复、外部 Skill 适配及安装回退边界保持不变。
...
- C10 复用中央启动图批准，按波次批量派发，强制绑定保存项目；worktree 项目 ID 丢失时对同一任务自动走项目本地往返交接，再验证真实运行归属；
