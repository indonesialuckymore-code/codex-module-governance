...
当前开发施工版为 `0.19.0`（尚未发布 Git 稳定标签）；它在中央启动图、稳定任务身份和角色连续性之上，新增“原生消息送达并由对方确认才算已收到”的双向通信桥、自动收件箱、一次只处理一项的独立验收槽、`DONE` 后自动续派、“每窗口最多两项串行任务”硬门禁，以及不降低验收标准的主动子 Agent 协作。
...
先按 [安装说明](docs/INSTALLATION.md) 从固定稳定标签注册本人 GitHub Marketplace，再安装 `codex-module-governance@qianyi-codex-governance`。私有仓库用户必须先获得 GitHub 访问权限；安装或升级后需要新开 Codex 任务加载新版能力。`0.19.0` 尚未打标签，当前只适合本地施工验收。
