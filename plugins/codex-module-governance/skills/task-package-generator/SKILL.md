...
5. 已接入新版大纲的任务，读取[大纲成果交接协议](../../references/outline-outcomes.md)。C04 自动携带 C03 已登记的目标、成果与可用标准，不能在 brief 中另写一份相冲突的成果合同；无成果关联的旧任务保持兼容，不自动补写历史。
6. 新任务按[按成果和实际影响验收](../../references/adaptive-acceptance.md)选验收方法：共同质量底线保留，不适用项提前给出业务理由，随同一任务合同批准；不把软件测试清单强套到所有项目，也不在验收时临时减标准。
...
- 成果可用标准及与实际影响相称的检查；适用时包括正反例、重复效果、恢复、来源/历史和上下游读回，不适用时按合同说明。
