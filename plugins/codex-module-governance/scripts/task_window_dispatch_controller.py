...
SUB_AGENT_PERMISSION_METHODS = {
    "INHERITED_FROM_PARENT_WINDOW",
    "SUB_AGENT_PERMISSION_READBACK",
}
...
def prepare(args: argparse.Namespace) -> Tuple[Dict[str, Any], int]:
    ...
    plan = {
        ...
        "nativeRuntime": {
            ...
            "readbackTool": "codex_app__read_thread",
            "renameTool": "codex_app__set_thread_title" if previous_task_id else None,
            "monitorTool": "codex_app__wait_threads",
            "maxMonitoringBatch": 8,
            "subAgentTool": "collaboration.spawn_agent",
...
def validate_confirmation(raw: Dict[str, Any], dispatch_id: str, plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for raw_agent in agents:
        agent = exact(raw_agent, {"subAgentId", "status", "runtimeAgentRef", "modelControl", "permissionControl"}, "C10_SUB_AGENT_CONFIRMATION_INVALID")
...
def confirm(args: argparse.Namespace) -> Tuple[Dict[str, Any], int]:
    ...
    if (
        ...
        raise DispatchError("C10_MANUAL_PERMISSION_EVIDENCE_REQUIRES_FALLBACK_PACKAGE")
