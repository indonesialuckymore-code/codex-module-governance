...
当前仓库已包含 C02 新项目初始化器、C03 Codex 模块工程总账和 C04 待审施工任务包。它们只能进行私有建档、账本记录、完整性校验和任务包草案生成，不能派工、创建 TEST 或开展业务施工。
...
项目启动卡不包含任务、窗口、证据或业务数据。C03 总账只保存私有的结构化施工状态和不透明引用；C04 任务包只保存去敏的施工合同与不透明引用，不包含真实业务对象。C13 完成前，完整安装流程仍仅用于开发预览，不能作为生产启用步骤。
