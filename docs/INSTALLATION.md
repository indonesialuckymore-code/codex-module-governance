...
当前仓库已包含 C02 新项目初始化器、C03 工程总账、C04 待审任务包、C05 占用检查、C06 独立回传验收、C07 裁定请求、C08 失联恢复和 C09 唯一中央路由。它们只能管理治理状态和引用，不能自行派工、创建 TEST 或开展业务施工。
...
10. C09 先验证唯一中央、C03 总账和 C08 恢复状态，再把 Boss 请求路由到 C02–C08；它不调用下游，C10 完成前所有真实派发都被拦截。
